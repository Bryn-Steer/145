...
import bg_img from "/assets/img/bg/hero_bg_1.png";
import { Link } from "react-router-dom";
const sliderData = [
  {
    title: "Henriette Petite",
    maintitle: "YOU WON’T LEAVE HUNGRY.",
    desp: "Great Coffee starts with the finest granules. We source only the freshest produce, highest quality beans, and artisanal products to create mornings that are bursting with flavour.",
    buttonUrl: "reservations",
  },
  {
    title: "Henriette Petite",
    maintitle: "YOU WON’T LEAVE HUNGRY.",
    desp: "Our food is made with love and care, using only the freshest ingredients. From our signature dishes to our daily specials, we have something for everyone. Whether you’re in the mood for a hearty breakfast, a light lunch, we’ve got you covered.",
    buttonUrl: "reservations",
  },
  {
    title: "Henriette Petite",
    maintitle: "YOU WON’T LEAVE HUNGRY.",
    desp: "Atmosphere is everything. We’ve created a warm and welcoming space that’s perfect for catching up with friends, enjoying a romantic dinner, or just treating yourself to a delicious meal.",
    buttonUrl: "reservations",
  },
];
                  ...
                  <SwiperSlide key={index}>
                    <div className="slider-info">
                      <div className="hero-title">
                        <h1 className="hero-main-title" style={{ textShadow: "0 3px 10px rgba(0,0,0,0.55)" }}> {item.title}</h1>
                        <h1 className="hero-main-title-1" style={{ textShadow: "0 2px 8px rgba(0,0,0,0.55)" }}> {item.maintitle}</h1>

                      </div>

                      <div className="ak-height-30 ak-height-lg-30"></div>
                      <div>
                        <p className="hero-sub-text" style={{ fontSize: "1.15rem", fontWeight: 350, color: "#ffffffff", textShadow: "0 2px 6px rgba(0,0,0,01.95)", lineHeight: "1.45", }}> {item.desp}</p>


                      </div>
                      <div className="ak-height-70 ak-height-lg-30"></div>
                      <Link to={item.buttonUrl} className="hero-btn style-1" >
                        <div className="ak-btn style-5 color-yellow-bg" style={{ fontSize: "1.15rem", fontWeight: 550, textShadow: "0 3px 10px rgba(126, 126, 126, 0.75)" }}>
                          {"Visit us Now"}
                        </div>
                      </Link>
                    </div>
